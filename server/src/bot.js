  ...
  bot.command('connect', async (ctx) => {
    ...
    await ctx.replyWithHTML(
      [
        ...
        `<code>${token}</code>`,
        '',
        '<b>1. Serverda ishga tushiring</b> — Node.js 20+ kerak:',
        `<code>npx ${agentPackage} start --key ${token}</code>`,
        '',
        'Agent shu serverdagi loyihalarni o‘zi topadi va qaysi birini ' +
          'kuzatishni shu guruhda so‘raydi.',
        '',
        '<b>2. Doimiy ishlashi uchun</b> (server o‘chib yonsa ham o‘zi tursin):',
        `<code>npm i -g ${agentPackage}</code>`,
        `<code>pingo init --key ${token}</code>`,
        '<code>pm2 start pingo -- start &amp;&amp; pm2 save</code>',
        '',
        'Tekshirish: <code>pingo doctor</code> — nima kuzatilishi mumkinligini ko‘rsatadi.',
        '',
